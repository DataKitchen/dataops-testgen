...
Like data_contract_saved_version.py but with pending edits pre-seeded in session
state via dc_test_pending_count (count) and dc_test_pending_data (dict).
Tests read dc_pending:{TG_ID} after rendering to verify round-trip.
"""
...
# ---------------------------------------------------------------------------
# Seed the version record so load_contract_version is not called and our
# pre-seeded YAML / pending are used without a DB round-trip.
# ---------------------------------------------------------------------------
pending_key = f"dc_pending:{TG_ID}"
yaml_key    = f"dc_yaml:{TG_ID}"
version_key = f"dc_version:{TG_ID}"

# Read test-controlled pending count from a side-channel key
...
# Seed yaml + version so page uses cached values
