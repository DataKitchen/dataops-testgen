"""
Streamlit app script for AppTest — Data Contract first-time flow.

Loaded by AppTest.from_file() in test_data_contract_apptest.py.
Each call to at.run() re-executes this script from top, re-applying
all patches and re-rendering the page.

Usage:
    from streamlit.testing.v1 import AppTest
    at = AppTest.from_file("tests/functional/ui/apps/data_contract_first_time_flow.py")
    at.run()
"""
...
# streamlit.components.v1.declare_component and v2 APIs are unavailable
# in the AppTest execution environment.
# ---------------------------------------------------------------------------
import streamlit.components.v1 as _stv1
_stv1.declare_component = MagicMock(return_value=MagicMock())

# Stub the low-level component utility module so nothing downstream tries
# to call declare_component when testgen.ui.components is first imported.
if "testgen.ui.components.utils.component" not in sys.modules:
    sys.modules["testgen.ui.components.utils.component"] = MagicMock()

# Stub testgen_component module so page_header's import succeeds.
...
# ---------------------------------------------------------------------------
# Mock objects (re-created each script run — that's fine for AppTest)
# ---------------------------------------------------------------------------
_mock_tg = MagicMock()
