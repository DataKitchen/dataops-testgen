...
const WizardProgressIndicator = (steps, currentStep) => {
  ...
  const currentStepIndicator = (title, stepIndex) => div(
    { class: `flex-column fx-align-flex-center fx-gap-1 ${currentPhysicalIndex === stepIndex ? 'step-icon-current' : ''}`, style: 'position: relative;' },
    stepIndex === 0
    ...
    stepIndex === steps.length - 1
      ...
      : '',
    div(
      { class: 'flex-row', style: `border: 2px solid ${colorMap.green}; background: var(--dk-card-background); border-radius: 50%;` },
      i(
        {
            class: 'material-symbols-rounded',
            style: `font-size: 20px; color: ${colorMap.green};`,
        },
        'more_horiz',
      ),
    ),
  ...
  return div(
    {
      ...
      class: 'flex-row fx-justify-space-between mb-2',
