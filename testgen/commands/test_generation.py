...
from testgen.common.read_file import get_template_files, read_template_sql_file
...
def run_test_generation(test_suite_id: str | UUID, generation_set: GenerationSet = "Standard") -> str:
    ...
    success = False
    try:
        TestGeneration(connection, table_group, test_suite, generation_set).run()
        success = True
    except Exception:
...
class TestGeneration:
    ...
    def __init__(self, connection: Connection, table_group: TableGroup, test_suite: TestSuite, generation_set: str):
    ...
    def run(self) -> None:
        ...
        test_types = fetch_dict_from_db(*self.get_selection_test_types())
        test_types = [TestTypeParams(**item) for item in test_types]

        LOG.info("Running test generation queries")
        execute_db_queries([
            *[self.generate_selection_test_type(tt) for tt in test_types],
            *self.generate_template_test_types("gen_funny_cat_tests"),
            *self.generate_template_test_types("gen_query_tests"),
            self.delete_old_tests(),
        ])

    def get_selection_test_types(self) -> tuple[str, dict]:
        # Runs on App database
        return self._get_query("get_selection_test_types.sql")
    
    def generate_selection_test_type(self, test_type: TestTypeParams) -> tuple[str, dict]:
        # Runs on App database
        return self._get_query("gen_selection_tests.sql", test_type=test_type)

    def generate_template_test_types(self, template_directory: str) -> list[tuple[str, dict]]:
        # Runs on App database
        queries = []
        try:
            queries.extend([
                self._get_query(file.name, template_directory)
                for file in get_template_files(r"^.*sql$", template_directory)
            ])
        except:
            LOG.debug(f"Test generation template folder '{template_directory}' does not exist")

        flavor_directory = f"flavors.{self.flavor}.{template_directory}"
        try:
            queries.extend([
                self._get_query(file.name, flavor_directory)
                for file in get_template_files(r"^.*sql$", flavor_directory)
            ])
        except:
            LOG.debug(f"Test generation template folder '{flavor_directory}' does not exist")
