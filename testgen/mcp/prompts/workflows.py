...
def profiling_overview() -> str:
    """Explore the profiling results for a table group — understand data shapes, types, null rates, and anomalies."""
    return """\
...
3. Call `list_profiling_summaries(table_group_id='...')` for the quality health overview (scores, anomaly counts, last profiled).
4. Call `get_table(table_group_id='...', table_name='...')` for structural metadata, the column list, and table-level highlights.
5. Call `list_column_profiles(table_group_id='...', table_name='...')` to scan all columns — datatype, null rates, distinct counts, quality scores, and anomaly counts per column.
