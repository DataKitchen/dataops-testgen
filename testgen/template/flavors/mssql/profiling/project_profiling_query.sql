  ...
  SELECT pattern,
    ...
    FROM ( SELECT pattern, COUNT(*) AS ct
             FROM ( SELECT TRANSLATE("{COL_NAME}"  COLLATE Latin1_General_BIN,
                             'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789', 'aaaaaaaaaaaaaaaaaaaaaaaaaaAAAAAAAAAAAAAAAAAAAAAAAAAANNNNNNNNNN' ) AS pattern
                      FROM target_table
...
-- TG-IF is_N_sampling
  , (SELECT TOP 1
        ...
        FROM "{DATA_SCHEMA}"."{DATA_TABLE}" TABLESAMPLE ({SAMPLE_PERCENT_CALC} PERCENT) {TABLE_HINT}) pctile
-- TG-ENDIF
-- TG-IF is_N_no_sampling
  , (SELECT TOP 1
             ...
             PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY "{COL_NAME}") OVER () AS pct_50,
             PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY "{COL_NAME}") OVER () AS pct_75
        FROM "{DATA_SCHEMA}"."{DATA_TABLE}" {TABLE_HINT}) pctile
-- TG-ENDIF
