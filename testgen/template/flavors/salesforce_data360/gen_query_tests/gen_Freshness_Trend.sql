  ...
  SELECT profile_run_id, schema_name, table_name,
    ...
    'CAST(COUNT(*) AS VARCHAR) || ''|'' || ' ||
      STRING_AGG(
        REPLACE(
          CASE
            WHEN general_type = 'D' THEN 'CAST(MIN(@@@) AS VARCHAR) || ''|'' || CAST(MAX(@@@) AS VARCHAR) || ''|'' || CAST(COUNT(DISTINCT @@@) AS VARCHAR)'
            WHEN general_type = 'A' THEN 'CAST(MIN(@@@) AS VARCHAR) || ''|'' || CAST(MAX(@@@) AS VARCHAR) || ''|'' || CAST(COUNT(DISTINCT @@@) AS VARCHAR) || ''|'' || CAST(SUM(LENGTH(@@@)) AS VARCHAR)'
            WHEN general_type = 'N' THEN 'CAST(COUNT(@@@) AS VARCHAR) || ''|'' ||
              CAST(COUNT(DISTINCT MOD(CAST(CAST(COALESCE(@@@,0) AS DECIMAL(38,6)) * 1000000 AS DECIMAL(38,0)), 1000003)) AS VARCHAR) || ''|'' ||
              COALESCE(CAST(CAST(MIN(@@@) AS DECIMAL(38,6)) AS VARCHAR), '''') || ''|'' ||
              COALESCE(CAST(CAST(MAX(@@@) AS DECIMAL(38,6)) AS VARCHAR), '''') || ''|'' ||
              COALESCE(CAST(MOD(COALESCE(SUM(MOD(CAST(ABS(COALESCE(@@@,0)) AS DECIMAL(38,6)) * 1000000, 1000000007)), 0), 1000000007) AS VARCHAR), '''') || ''|'' ||
              COALESCE(CAST(MOD(COALESCE(SUM(MOD(CAST(ABS(COALESCE(@@@,0)) AS DECIMAL(38,6)) * 1000000, 1000000009)), 0), 1000000009) AS VARCHAR), '''')'
          END,
          '@@@', '"' || column_name || '"'
        ),
        ' || ''|'' || '
