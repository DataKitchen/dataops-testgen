WITH stats
   AS (SELECT COUNT(*)::REAL as record_ct,
              ROUND(CAST({PROFILE_SAMPLE_PERCENT} as REAL) * CAST(COUNT(*) as REAL) / 100.0) as calc_sample_ct,
              CAST({PROFILE_SAMPLE_MIN_COUNT} as REAL) as min_sample_ct,
              CAST(999000 as REAL) as max_sample_ct
         FROM {SAMPLING_TABLE} )
