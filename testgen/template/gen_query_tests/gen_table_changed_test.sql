                  ...
                  ORDER BY
                     CASE
                       ...
                       WHEN functional_data_type = 'ID-Secondary' THEN 2
                        ELSE 3
                     END, distinct_value_ct DESC) AS rank
          FROM curprof
...
tran_date_cols
   AS ( SELECT profile_run_id, schema_name, table_name, column_name, functional_data_type, general_type,
               ...
               ROW_NUMBER() OVER (PARTITION BY schema_name, table_name
                  ORDER BY
                     distinct_value_ct DESC) AS rank
          FROM curprof
...
numeric_cols_ranked
   AS ( SELECT *,
               ROW_NUMBER() OVER (PARTITION BY schema_name, table_name
                  ORDER BY change_detection_score DESC) as rank
          FROM numeric_cols
         WHERE change_detection_score IS NOT NULL),
combined
   AS ( SELECT profile_run_id, schema_name, table_name, column_name, 'ID' AS element_type, general_type, rank AS fingerprint_order
          FROM id_cols
         WHERE rank <= 2
         UNION ALL
        SELECT profile_run_id, schema_name, table_name, column_name, 'DATE_P' AS element_type, general_type, rank AS fingerprint_order
          FROM process_date_cols
         WHERE rank = 1
         UNION ALL
        SELECT profile_run_id, schema_name, table_name, column_name, 'DATE_T' AS element_type, general_type, rank AS fingerprint_order
          FROM tran_date_cols
         WHERE rank = 1
         UNION ALL
        SELECT profile_run_id, schema_name, table_name, column_name, 'MEAS' AS element_type, general_type, rank AS fingerprint_order
   ...
   AS (SELECT profile_run_id, schema_name, table_name,
             ...
             STRING_AGG(
                REPLACE(
                        ...
                        '@@@', '"' || column_name || '"'),
                        ' || ''|'' || '
                        ORDER BY element_type, fingerprint_order) as fingerprint
        FROM combined
