...
profiling_records AS (
    SELECT
        ...
        SUM(weighted_record_ct) AS data_point_ct,
        SUM(weighted_record_ct * good_data_pct) / NULLIF(SUM(weighted_record_ct), 0) AS score
    FROM v_dq_profile_scoring_latest_by_impact_dimension
    WHERE {filters}
...
test_records AS (
    SELECT
        ...
        {columns},
        SUM(issue_ct) AS issue_ct,
        SUM(weighted_dq_record_ct) AS data_point_ct,
        SUM(weighted_dq_record_ct * good_data_pct) / NULLIF(SUM(weighted_dq_record_ct), 0) AS score
    FROM v_dq_test_scoring_latest_by_impact_dimension
    WHERE {filters}
    GROUP BY project_code, {columns}
),
parent AS (
    SELECT
        ...
        SUM(COALESCE(profiling_records.weighted_record_ct, 0)) AS profiling_data_points,
        SUM(COALESCE(test_records.weighted_dq_record_ct, 0)) AS test_data_points
